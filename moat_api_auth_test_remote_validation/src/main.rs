...
async fn main() -> std::io::Result<()> {
    ...
    let key_handler = Arc::new(RemoteKeyHandler::init2(valami(http_client.clone())).await);
...
async fn valami(http_client: Arc<reqwest::Client>) -> Result<String, String> {
    let result = http_client
        .get("http://localhost:8080/auth/jwk")
        .send()
        .await;
    match result {
